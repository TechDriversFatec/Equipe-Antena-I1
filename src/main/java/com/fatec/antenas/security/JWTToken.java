...
public class JWTToken {
		...
		public boolean jwtGenerate(HttpServletResponse response, DocumentEmpresario findEmpresario, Usuario empresario) {
			BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
			if(findEmpresario == null) {
				
				throw new CredentialNotFoundException("User not Found ");
			}
			
			if(!passwordEncoder.matches(empresario.getSenha(), findEmpresario.getSenha())) {
				throw new CredentialNotFoundException("User not Found ");
			}
			
			String jwt = JWT.create()
	                .withClaim("idUsuarioLogado", findEmpresario.get_id())
